...
if (config.CopyBundles === false)
{
    config.IgnoreList.push("bundles/")
}

const exclude = glob.sync(`{${config.IgnoreList.join(",")}}`, { realpath: true, absolute: true, dot: true });

fs.copySync(currentDir, path.normalize(`${currentDir}/../~${modName}`), {
    filter: (src) =>
    {
        const shouldExclude = exclude.some((pattern) => minimatch(src, pattern));
